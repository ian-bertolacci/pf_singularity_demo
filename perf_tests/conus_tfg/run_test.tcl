...
proc run_test { test_directory P Q R T } {
  ...
  for { set i 1 } { $i <= $number_of_runs } { incr i } { 
    ...
    file delete -force validate_results.tcl
    cd $cwd
  }
  puts [pwd]
  ...
  file delete -force $test_run_dir/pfbdiff.py
  if { [file exists $test_run_dir/validate_results.tcl] == 1 } {
    file delete -force $test_run_dir/validate_results.tcl
   }
}
